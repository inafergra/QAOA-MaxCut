...
def circuit_ansatz(G, gamma, beta): # , p=1): #gamma and beta are p-lists
    ...
    for edge in E:
        k = edge[0]
        l = edge[1]
        QAOA.cu1(-2*gamma, k, l) #Controlled-Z gate with a -2*gamma phase
        QAOA.u1(gamma, k) #Rotation of gamma around the z axis
        QAOA.u1(gamma, l) #
    QAOA.barrier()
    QAOA.rx(2*beta, range(n)) #X rotation
    QAOA.barrier()
...
def execute_circuit(G, gamma, beta, backend, noise_model = None): 
...
def expect_value_function(parameters, backend, G, noise_model = None):
    gamma = parameters[0]
    beta = parameters[1]
    state_dictionary = execute_circuit(G, gamma, beta, backend, noise_model = noise_model)
...
print('before minizing')
# Nelder-Mead optimizer:
max_expect_value = minimize(expect_value_function, x0=np.random.randn(2), args=(backend,G), options={'disp': True}, method = 'Nelder-Mead')
